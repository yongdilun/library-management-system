...
def ensure_server():
    ...
    process = subprocess.Popen(
        [sys.executable, "-m", "flask", "--app", ".\\app.py", "run", "--host", "127.0.0.1", "--port", "5000"],
        cwd=str(ROOT),
        env=os.environ.copy(),
...
def safe_click(driver, by, value, timeout=3):
    try:
...
def safe_clear(driver, by, value, timeout=3):
    try:
...
def open_profile_tab(driver):
    page(driver, "/user/")
    return safe_click(driver, By.ID, "profile-tab")
...
def run_tests(driver, data, rec):
    ...
    reset_profile_user()
    post_profile("SVV Profile", "ali.com")
    email_after = scalar("SELECT email FROM users WHERE id=%s", (data["profile_user_id"],))
    rec.add("TC-04-002", email_after == "svv_profile@example.test", "Postman/HTTP", "Invalid email was submitted through the profile update route.")
    ...
    page(driver, "/")
    cleared = safe_clear(driver, By.NAME, "keyword")
    if cleared:
        click_submit(driver)
    rec.add("TC-07-002", cleared and ("validation" in driver.page_source.lower() or "required" in driver.page_source.lower()), "Selenium", "Empty search input was submitted from the home search form.")
    ...
    if cleared:
        click_submit(driver)
    rec.add("TC-07-006", cleared and ("validation" in driver.page_source.lower() or "required" in driver.page_source.lower()), "Selenium", "Empty search validation message was checked.")

    # F008 Reserve Book
    login_user(driver, "svv_user@example.test", "password")
    page(driver, f"/books/{data['available_book_id']}")
    clicked_add = safe_click(driver, By.LINK_TEXT, "Add")
    reserved = scalar(
        "SELECT COUNT(*) FROM reserve WHERE user_id=%s AND book_id=%s",
        (data["user_id"], data["available_book_id"]),
    )
    rec.add("TC-08-001", clicked_add and reserved == 1 and "Book reserved" in driver.page_source, "Selenium", "Available book was reserved through the UI.")
    ...
    rec.add("TC-08-002", clicked_add and unavailable_reserved == 0 and "unavailable" in driver.page_source.lower(), "Selenium", "Unavailable book reservation was attempted through the UI.")
    ...
    rec.add("TC-08-003", clicked_add and limit_reserved == 0 and "limit" in driver.page_source.lower(), "Selenium", "A user with five reservations attempted another reservation.")
